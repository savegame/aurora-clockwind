...
GJoystick = {
	["area"] = { ["x"] = 0, ["y"] = buttonsize, ["w"] = math.floor(WINWIDTH * 0.5), ["h"] = WINHEIGHT - buttonsize },
	["id"] = nil, -- touch id
	["presspos"] = { ["x"] = 0, ["y"] = 0 }, -- press position
	["pos"] = { ["x"] = 0, ["y"] = 0 }, -- current position
	["radius"] = buttonsize, -- joystick radius
	["stick_radius"] = buttonsize * 0.5 -- stick radius
}

GButtons = {
	-- keys outside of screen =) (only for joystic call this)
	["left"] = {0 - WINWIDTH, WINHEIGHT - buttonsize * 2, nil, nil},
	["right"] = { 0 + buttonsize * 2- WINWIDTH, WINHEIGHT - buttonsize*2, nil, nil},
	["up"] = { 0 + buttonsize- WINWIDTH, WINHEIGHT - buttonsize*3, nil, nil},
	["down"] = { 0 + buttonsize- WINWIDTH, WINHEIGHT - buttonsize, nil, nil},
	-- real screen keays
	["joystick"] = GJoystick,
	["jump"] = {WINWIDTH - buttonsize-4, WINHEIGHT - buttonsize - 4,nil, nil},
	["action"] = {WINWIDTH - (buttonsize+4)* 2, WINHEIGHT - buttonsize - 4, nil, nil},
	["timeleft"] = {WINWIDTH - buttonsize-4, WINHEIGHT - (buttonsize + 4)*2.5, nil, nil},
	["timeright"] = {WINWIDTH - buttonsize-4, WINHEIGHT - (buttonsize + 4)*3.5, nil, nil},
	["escape"] = {4, 4, nil, nil}
}

local getbutton = function (x, y)
	for k, v in pairs(GButtons) do
		-- print("v = "..v[1].." "..v[2])
		if k == "joystick" then
			if --GJoystick.id == nil and
					v.area.x <= x 
				and v.area.y <= y 
				and v.area.x + v.area.w >= x 
				and v.area.y + v.area.h >= y then
				return k
			end
		else
			if v[1] < x 
				and v[2] < y 
				and x < v[1] + buttonsize 
				and y < v[2] + buttonsize then
			-- print("catch "..k)
				return k
			end
		end
...
local getbuttonbyid = function (id)
	for k, v in pairs(GButtons) do
		-- print("v = "..v[1].." "..v[2])
		-- if k == "joystick" and v.id == id then
			-- return k
		if v[3] == id then
			return k
		end
...
function game.load(level)
	...
	GButtons.escape[4] = love.graphics.newImage("graphics/buttons/back.png")
	GButtons.action[4] = love.graphics.newImage("graphics/buttons/hand.png")
	GButtons.jump[4] = love.graphics.newImage("graphics/buttons/jump.png")
	GButtons.timeright[4] = love.graphics.newImage("graphics/buttons/clockright.png")
	GButtons.timeleft[4] = love.graphics.newImage("graphics/buttons/clockleft.png")
end
...
function game.update(dt)
	...
	local timeleftTouched = (GButtons.timeleft[3] ~= nil) or love.keyboard.isDown(controls["timeleft"])
	local timerightTouched = (GButtons.timeright[3] ~= nil) or love.keyboard.isDown(controls["timeright"])

	--time travel
	if not leveledit then
		local oldtimep = timep
		if (timeleftTouched or (love.keyboard.isDown(controls["timetoggle"]) and love.keyboard.isDown(controls["left"]))) and (not (timep-math.max(1, timep - TIMETRAVELSPEED*dt) == 0)) and obj["player"][1].controlsenabled then
			timep = math.max(1, timep - TIMETRAVELSPEED*dt)
			timetraveling = TIMETRAVELANIMTIME
		elseif ( timerightTouched or (love.keyboard.isDown(controls["timetoggle"]) and love.keyboard.isDown(controls["right"]))) and (not (timep-math.min(TIMEPERIODS, timep + TIMETRAVELSPEED*dt) == 0)) and obj["player"][1].controlsenabled then
...
function game.draw()
	...
	for k, v in pairs(GButtons) do
		...
		elseif GJoystick.id ~= nil then
			-- joyarea pos
			local x = GJoystick.presspos.x - GJoystick.radius
			local y = GJoystick.presspos.y - GJoystick.radius
			-- joyarea size
			local asz = GJoystick.radius * 2
			-- stick pos
			local scorner = GJoystick.stick_radius
			local sx = GJoystick.pos.x - scorner
			local sy = GJoystick.pos.y - scorner
			local ssz = scorner * 2
			
			-- local corner = GJoystick.radius
			love.graphics.setColor(0.9,0.4,0.9,0.4)
			love.graphics.rectangle("fill",x,y,asz,asz, GJoystick.radius, GJoystick.radius, 8 )

			love.graphics.setColor(0.2,0.2,1.0,0.6)
			love.graphics.rectangle("fill",sx,sy,ssz,ssz, scorner, scorner, 8 )
		end
	end
end
...
function game.touchpressed(id, x, y, dx, dy, pressure)
	local currentbutton = getbutton(x,y)
	if currentbutton == "unhandled" then
		return
	end
	if currentbutton == "escape" then
		GButtons[currentbutton][3] = id
		game.keypressed("escape")
	elseif currentbutton == "joystick" then
		GJoystick.id = id
		GJoystick.presspos.x = x
		GJoystick.presspos.y = y
		GJoystick.pos.x = x
		GJoystick.pos.y = y
	else
		GButtons[currentbutton][3] = id
		game.keypressed(controls[currentbutton])
	end
end
...
function game.touchreleased(id, x, y, dx, dy, pressure)
	...
	if GJoystick.id == id then
		...
		for i,k in ipairs({"left", "right", "up", "down" }) do
			if GButtons[k][3] ~= nil then
				GButtons[k][3] = nil
				game.keyreleased(controls[k])
			end
		end
	elseif currentbutton ~= "joystick" then
		GButtons[currentbutton][3] = nil
	end
	if currentbutton ~= "escape" then
		game.keyreleased(controls[currentbutton])
	end
end
...
function game.touchmoved(id, x, y, dx, dy, pressure)
	...
	-- if currentbutton == "unhandled" then
		-- return
	-- end
	-- if currentbutton == "joystick" then
	if GJoystick.id == id then
		GJoystick.pos.x = x
		GJoystick.pos.y = y
		local vx = 1.0 * (x - GJoystick.presspos.x)
		local vy = 1.0 * (y - GJoystick.presspos.y)
		local ny = vy
		local len = math.sqrt(vx*vx + vy*vy)
		local max_len = ( GJoystick.radius - GJoystick.stick_radius)
		if len > max_len then
			vx = vx / len * max_len
			ny = vy / len
			vy = ny * max_len
			GJoystick.pos.x = GJoystick.presspos.x + vx
			GJoystick.pos.y = GJoystick.presspos.y + vy
			len = max_len
		end
		for i,k in ipairs({"left", "right", "up", "down" }) do
			if GButtons[k][3] ~= nil then
		...
		if len > GJoystick.radius * 0.15 then
			...
			if	angle >=  0.25 and angle < 0.75 then
				GButtons.down[3] = id
				game.keypressed(controls["down"])
			elseif	angle >= 0.75 or angle <= -0.75 then
				GButtons.left[3] = id
				game.keypressed(controls["left"])
			elseif (angle <=  0.25 and angle > -0.25 ) then
				GButtons.right[3] = id
				game.keypressed(controls["right"])	
			elseif	angle <= -0.25 and angle > -0.75 then
				GButtons.up[3] = id
				game.keypressed(controls["up"])
			end
		end
	end
end
