...
settingsmenu = {
	...
	change = function(a) volume = math.floor(math.max(0, math.min(2, volume+(a*0.1)))*100)/100; love.audio.setVolume(volume) end, --change (a is -1 or 1)
...
function menu.touchpressed(id, x, y, dx, dy, pressure)
	...
	if menustate == "title" then
		--title screen
		-- print(" in titile touched "..x..":"..y)

		for i, s in pairs(titlemenu) do
			local name = s
			local rx, ry, rw, rh = math.floor((WINWIDTH-90)/2), 122+21*(i-1), 90, 20
			if x >= rx and y >= ry and x <= rx + rw and y <= ry + rh then
				titleselection = i
				-- print("selected "..i)
				menu.keypressed(controls["jump"])
				break
			-- else
				-- print("rx.ry = "..rx.." "..ry.." w.h = "..rw.." "..rh)
			end
		end
	elseif menustate == "settings" then
		--format the list of settings
		-- print(" in settings touched "..x..":"..y)
		for i, s in pairs(settingsmenu) do
			local name = s[1]
			local selcolor = {1,1,1,1}

			local rx, ry, rw, rh =  (WINWIDTH-190)/2, 17+21*(i-1), 190, 20
			if x >= rx and y >= ry and x <= rx + rw and y <= ry + rh then
				settingsselection = i
				-- print("selected "..i)
				if x < rx + rw * 0.5 then
	...
	elseif menustate == "controls" then
		--format the list of controls
		-- print(" in controls touched "..x..":"..y)

		for i, s in pairs(controlsmenu) do
			local name = s[1]
			local rx, ry, rw, rh =  (WINWIDTH-200)/2, 8+16*(i-1), 200, 18

			if x >= rx and y >= ry and x <= rx + rw and y <= ry + rh then
				controlsselection = i
				if string.lower(name) == "apply" then
					menu.keypressed(controls["jump"])
				end
			end
		end
