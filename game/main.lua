...
local draw_canvas = function (canvas, x, y, r, sx, sy)
...
if love.system.getOS() == "SailfishOS" then
	draw_canvas = function (canvas, x, y, r, sx, sy)
		local nx = love.graphics.getWidth();
		local ny = 0;
		local nr = math.pi * 0.5
		game_orientation = "landscape"
		if love.window.getDisplayOrientation() == "landscapeflipped" then
			nr = math.pi * 1.5
			nx = 0
			ny = love.graphics.getHeight()
			game_orientation = love.window.getDisplayOrientation();
		end
		love.graphics.draw(canvas, nx, ny, nr, love.graphics.getHeight()/WINWIDTH, love.graphics.getWidth()/WINHEIGHT)
	end
end
...
function love.draw()
	if usecanvas then
		...
		-- love.graphics.draw()
		draw_canvas(canvas, 0, 0, 0, love.graphics.getHeight()/WINWIDTH, love.graphics.getWidth()/WINHEIGHT)
		love.graphics.setBlendMode("alpha")
	else
		-- print(("Screen size %2ix%2i"):format(love.graphics.getWidth(), love.graphics.getHeight()))
		-- local scale = 1.25 --math.min(love.graphics.getWidth()/WINWIDTH, love.graphics.getHeight()/WINHEIGHT)
...
function load_settings()
	if love.filesystem.exists("settings") then 
		local s2
		local sfile, size = love.filesystem.read("settings")
		s2 = string.split(sfile,"~")
