...
function Player:update(dt)
	...
	local leftTouched = (GButtons.left[3] ~= nil or love.keyboard.isDown(controls["left"]))
	local rightTouched = (GButtons.right[3] ~= nil or love.keyboard.isDown(controls["right"]))
	local upTouched = (GButtons.up[3] ~= nil or love.keyboard.isDown(controls["up"]))
	local downTouched = (GButtons.down[3] ~= nil or love.keyboard.isDown(controls["down"]))
	local timeleftTouched = (GButtons.timeleft[3] ~= nil)
	local timerightTouched = (GButtons.timeright[3] ~= nil)
	local istoggletime = ( timeleftTouched or timerightTouched or love.keyboard.isDown(controls["timetoggle"]) )
	...
	if self.car then
		if timeperiod == 3 then
			...
			local isLeftDown = (GButtons.left[3] ~= nil)
			if isLeftDown then
				print("Left is down")
			else 
				print( "GButtons " .. tostring(GButtons.left[3]) )
			end
			if self.controlsenabled and (upTouched and not love.keyboard.isDown(controls["timetoggle"])) then
			...
			elseif self.controlsenabled and (downTouched and not love.keyboard.isDown(controls["timetoggle"])) then
				if self.sy < 0 then
	...
	if self.gear then
		...
		-- print( "Is left touched " .. tostring(GButtons.left[3]) )
		--normal movement
		if self.controlsenabled and ( leftTouched and not istoggletime ) then--and self.dir == "left" then
			self.dir = "left"
			if self.swimming then
		...
		elseif self.controlsenabled and ( rightTouched and not istoggletime ) then--and self.dir == "right" then
			self.dir = "right"
			if self.swimming then
